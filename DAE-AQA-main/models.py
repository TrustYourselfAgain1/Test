...
class DAE(nn.Module):
    def __init__(self):
        ...
        self.fc2_mean = nn.Linear(128, 1)  # 第三个全连接层，输入维度为128，输出维度为1，用来学习潜在空间中样本的均值
        self.fc2_logvar = nn.Linear(128, 1)  # 第四个全连接层，输入维度为128，输出维度为1，用于输出编码后的对数方差

    # 定义了编码器的前向传播过程。接受输入x，经过两个全连接层和激活函数ReLU后，将输入数据 x编码成潜在空间中的均值mu和对数方差logvar
    def encode(self, x):
        h0 = F.relu(self.fc1(x))
        h1 = F.relu(self.fch(h0))
        return self.fc2_mean(h1), self.fc2_logvar(h1)

    # 接受均值mu和对数方差logvar，生成并返回从正态分布中采样的隐变量z 确保模型在训练过程中可以通过反向传播来学习潜在变量的分布
    def reparametrization(self, mu, logvar):
        std = logvar.mul(0.5).exp_()  # 计算标准差
        # return torch.normal(mu, std)
        esp = torch.randn(*mu.size()).cuda()  # 是用来生成一个与mu张量相同形状的随机张量。这个张量是从标准正态分布（均值为0，方差为1）中随机采样得到的
        z = mu + std * esp
        return z
